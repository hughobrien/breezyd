...
**Tech Stack:** Go 1.26 + stdlib `net/http` + `templ` (codegen, generated files NOT committed) + vendored htmx 2.0.4 + response-targets extension + Playwright + pnpm.
...
**Goal:** `templ` available in `nix develop` and as a build input; `*_templ.go` ignored; `just generate` recipe runs codegen; `just check` enforces drift gate.
...
- [ ] `nix build` still succeeds (templ becomes a `nativeBuildInput` and `preBuild` runs `templ generate`, but at this task there are no `.templ` files yet so it's a no-op)
- [ ] `just generate` exists and runs `templ generate ./...`
- [ ] `just check` includes a drift step that fails if `templ generate` produces diffs
- [ ] `.gitignore` excludes `cmd/breezyd/ui/templates/*_templ.go`
...
Add `pkgs.templ` to both the devshell `packages` list and a new `nativeBuildInputs` line on the `breezyd-pkg` derivation, plus a `preBuild` that runs `templ generate`. Specifically, change `breezyd-pkg = pkgs.buildGoModule { ... }`:
...
Then update the `build`, `check`, `check-all`, and `ci` recipes to depend on `generate`. Replace the existing `build:` recipe block with:
...
And update the aggregate gates:
...
# templ-generated Go files (regenerated by `just generate`)
cmd/breezyd/ui/templates/*_templ.go
```
...
git commit -m "build: add templ to flake + justfile + gitignore (#14)"
```
